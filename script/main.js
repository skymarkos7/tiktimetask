    ...
    function pauseTimer(icon) {
      ...
      if (parseFloat(sgp) <=0.0) {
        ...
        sgpLogCell.setAttribute('title', 'Quantidade de tempo irrelevante para esse itém');
        sgpLogCell.addEventListener('click', openModal);
      } else {
        sgpLogCell.textContent = sgp;
        sgpLogCell.setAttribute('title', 'Clique para obter a tabela de conversão - valores arredondados');
        sgpLogCell.addEventListener('click', openModal);
      }
  ...
  const timeLogBody = document.getElementById('time-log-body');
  const descricaoClicada = event.target.parentNode.querySelector('td:nth-child(3)').textContent;
